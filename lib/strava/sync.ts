...
export async function backfillActivities(athleteId: string, years = 2) {
  ...
  const after = Math.floor(
    (Date.now() - years * 365 * 24 * 60 * 60 * 1000) / 1000,
  );
  const perPage = 200;
  let page = 1;
  let totalInserted = 0;
...
export async function fetchActivityStreams(
  ...
  if (Array.isArray(raw)) {
    ...
    for (const [k, v] of Object.entries(
      raw as Record<string, { data: unknown }>,
    )) {
      byType.set(k, v);
    }
  }
  ...
  const row = {
    ...
    velocity_smooth_data:
      (byType.get("velocity_smooth")?.data as number[]) ?? null,
...
export async function syncGear(athleteId: string) {
  ...
  // ones referenced by recently-synced activities — otherwise a shoe's total
  // goes stale the moment it drops out of the athlete's active rotation
  // (e.g. its last activity gets reassigned to a different shoe).
  const { data: knownGear } = await supabase
    .from("gear")
  ...
  const { data: localShoes } = await supabase
    .from("gear")
    .select(
      "id, name, brand_name, model_name, distance_m, baseline_m, nickname, primary_shoe, retired",
    )
    .eq("athlete_id", athleteId)
    .like("id", "local_%");
  ...
  for (const id of ids) {
    ...
    const baseline =
      baselineById.get(id) ??
      Number(local?.baseline_m ?? local?.distance_m ?? 0);
    ...
    if (local && local.id !== g.id) {
      ...
      console.log(
        `[gear] merged local shoe "${g.name}" (${local.id}) → Strava ${g.id}`,
      );
    }
  }
}
