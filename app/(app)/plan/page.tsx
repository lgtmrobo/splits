...
export default async function PlanPage({
  ...
  searchParams: { view?: string; plan?: string };
}) {
  const view =
    (searchParams.view as "week" | "block" | "full" | undefined) ?? "week";
  const requestedPlanId = searchParams.plan ?? null;
  const activePlan = await getActivePlan();
  const plan = requestedPlanId
    ? await getPlanById(requestedPlanId)
    : activePlan;
  if (!plan) {
    return (
      <div className="content fadein">
        <div className="muted" style={{ padding: 40, textAlign: "center" }}>
          {requestedPlanId ? "Plan not found." : "No active training plan."}
        </div>
      </div>
    );
  }
  ...
  const goalRace = plan.goal_race_id
    ? await getRaceById(plan.goal_race_id)
    : null;
  const coursePoints =
    goalRace?.course_polyline != null
      ? (decodePolyline(goalRace.course_polyline).map(([lat, lng]) => [
          lng,
          lat,
        ]) as [number, number][])
      : [];
  ...
  const peakWeek = weekMileage.reduce<{
    label: string;
    miles: number;
    start_date: string;
  } | null>((best, w) => {
    const mi = metersToMiles(w.planned_m);
    if (!best || mi > best.miles)
      return { label: w.label, miles: mi, start_date: w.start_date };
    return best;
  }, null);

  const weeksRemaining = Math.max(
    0,
    meta.total_weeks - meta.current_week_index - 1,
  );
  const upcomingPlanned = await getPlannedRunsBetween(
    baseWeek,
    plan.end_date,
    plan.id,
  );
  ...
  const longRunsRemaining = upcomingPlanned.filter(
    (p) => p.workout_type === "long",
  ).length;
  const longestUpcoming = upcomingPlanned.reduce<PlannedRun | null>(
    (best, p) =>
      !best ||
      Number(p.target_distance_m ?? 0) > Number(best.target_distance_m ?? 0)
        ? p
        : best,
    null,
  );
  const longestUpcomingWeek = longestUpcoming
    ? weekMileage.find(
        (w) =>
          w.start_date <= longestUpcoming.scheduled_date &&
          longestUpcoming.scheduled_date <= addDays(w.start_date, 6),
      )
    : null;
  ...
  if (peakWeek) {
    const peakIdx = weekMileage.findIndex(
      (w) => w.start_date === peakWeek.start_date,
    );
    for (let i = peakIdx + 1; i < weekMileage.length; i++) {
      if (metersToMiles(weekMileage[i].planned_m) < peakWeek.miles * 0.75) {
        taperStart = {
          label: weekMileage[i].label,
          date: weekMileage[i].start_date,
        };
        break;
      }
    }
  ...
  const thisWeekPlannedMi = weekView.reduce(
    (acc, d) =>
      acc + (d.planned ? metersToMiles(d.planned.target_distance_m ?? 0) : 0),
    0,
  );
  ...
  const thisWeekHas = weekView.filter(
    (d) => d.planned && d.planned.workout_type !== "rest",
  ).length;
  const thisWeekCutback =
    weekMileage.find((w) => w.start_date === thisWeekStart)?.is_cutback ??
    false;

  const nextWeekTotalMi = nextWeekRuns.reduce(
    (a, p) => a + metersToMiles(p.target_distance_m ?? 0),
    0,
  );

  const onPaceFinishMi =
    totalActualMi +
    (totalPlannedMi - totalActualMi) * (meta.adherence_pct / 100);
  ...
  return (
    <div className="content fadein">
      ...
      <div className="card" style={{ marginBottom: 14 }}>
        <div className="row between" style={{ marginBottom: 6 }}>
          <div className="col gap-4">
            <div className="stat-label" style={{ marginBottom: 0 }}>
              Cumulative Mileage · Plan vs Actual
            </div>
            <div className="row gap-14 baseline" style={{ marginTop: 6 }}>
              ...
              {deficitMi > 0 && (
                <Pill kind="warn">−{deficitMi.toFixed(1)} mi vs plan</Pill>
              )}
              {deficitMi <= 0 && totalActualMi > 0 && (
                <Pill kind="accent">
                  on pace · {Math.round(onPaceFinishMi)}mi at finish
                </Pill>
              )}
            </div>
          </div>
          <div className="row gap-14" style={{ flexShrink: 0 }}>
            {peakWeek && (
              ...
              >
                ...
                <span className="num" style={{ fontSize: 16 }}>
                  {formatMilesCompact(peakWeek.miles)} mi · {peakWeek.label}
                </span>
              </div>
            )}
            <div className="vr" />
            {taperStart ? (
              ...
              >
                ...
                <span className="num" style={{ fontSize: 16 }}>
                  {taperStart.label} · {fmtMd(taperStart.date)}
                </span>
              </div>
            ) : (
              <div
      ...
      {view === "block" && (
        <div className="card" style={{ marginBottom: 14 }}>
          <CardHeader
            title="All Weeks · Block view"
            action={`${weekMileage.length} weeks`}
          />
          <div className="col gap-6">
            {weekMileage.map((w) => {
              const planned = metersToMiles(w.planned_m);
              const actual =
                w.actual_m != null ? metersToMiles(w.actual_m) : null;
              ...
              return (
                ...
                >
                  <span style={{ width: 56, color: "var(--text-3)" }}>
                    {w.label}
                  </span>
                  <span style={{ width: 110, color: "var(--text-2)" }}>
                    {fmtRange(w.start_date)}
                  </span>
                  <span
                    className="num"
                    style={{
                  ...
                  >
                    ...
                    <span className="muted">
                      / {formatMilesCompact(planned)} mi
                    </span>
                  </span>
                  {w.is_cutback && <Pill kind="muted">Cutback</Pill>}
                  {isCurrent && <Pill kind="accent">Now</Pill>}
                  {isPast && actual != null && actual >= planned * 0.95 && (
                    <Pill kind="accent">✓</Pill>
                  )}
                </div>
              );
            })}
          ...
          {weekMileage.map(async (w) => {
            ...
            return (
              <div key={w.start_date} className="card">
                <CardHeader
                  title={`${w.label} · ${fmtRange(w.start_date)}${w.is_cutback ? " · cutback" : ""}`}
      ...
      {view === "week" && (
        ...
        >
          <div className="card">
            <CardHeader
              title={`This Week · ${fmtRange(thisWeekStart)}${thisWeekCutback ? " · cutback" : ""}`}
        ...
        <div className="card">
          ...
          >
            {Array.from({ length: 7 }).map((_, i) => {
              ...
              const miles = p?.target_distance_m
                ? metersToMiles(p.target_distance_m)
                : 0;
              return (
                <div
                  key={date}
  ...
  return (
    ...
    >
      <div className="row between" style={{ alignItems: "baseline" }}>
        ...
        {isToday && (
          <span className="pill accent" style={{ padding: "2px 6px" }}>
            Today
          </span>
        )}
        {!isToday && travelTag && (
