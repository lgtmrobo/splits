...
export default async function ShoesPage() {
  ...
  const lifetimeMi = Math.round(
    gear.reduce((acc, g) => acc + metersToMiles(g.distance_m), 0),
  );

  // Per-shoe aggregations from real activities
  const perShoe = new Map<
    string,
    {
      runs: number;
      distSum: number;
  ...
  for (const a of activities) {
    ...
    if (a.max_speed_ms && a.max_speed_ms > s.fastestSpeed)
      s.fastestSpeed = a.max_speed_ms;
    ...
    if (t >= week0) {
      ...
      if (idx >= 0 && idx < 12)
        s.weekly[idx] += metersToMiles(Number(a.distance_m ?? 0));
    }
  }

  // Top-line summary cards
  const mostMiles =
    [...gear].sort((a, b) => b.distance_m - a.distance_m)[0] ?? null;
  ...
  const readyRetireRemain = readyRetire.length
    ? Math.round(
        metersToMiles(
          readyRetire.reduce((a, g) => a + (g.cap_m - g.distance_m), 0),
        ),
      )
    : 0;
    ...
    <div className="content fadein">
      <div className="row between" style={{ marginBottom: 14 }}>
        <div className="col gap-4">
          ...
          <div className="muted num" style={{ fontSize: 12 }}>
            ...
            {lifetimeMi.toLocaleString()} lifetime mi
          </div>
        </div>
      ...
      >
        <div className="card compact">
          {mostMiles ? (
            <Stat
              label="Most miles"
              value={formatMilesHalf(metersToMiles(mostMiles.distance_m))}
              unit="mi"
              delta={mostMiles.name}
            />
          ) : (
        ...
        <div className="card compact">
          {fastestShoe ? (
            <Stat
              label="Fastest pace"
              value={fastestShoe.pace}
              unit="/mi"
              delta={fastestShoe.name}
            />
          ) : (
        ...
        <div className="card compact">
          <Stat
            ...
            delta={
              readyRetire.length ? `${readyRetireRemain} mi left` : undefined
            }
        ...
        <div className="card compact">
          <Stat
            label="This month"
            value={monthMi.toFixed(1)}
            unit="mi"
            delta={
              monthShoes.size
                ? `across ${monthShoes.size} shoe${monthShoes.size === 1 ? "" : "s"}`
                : undefined
            }
          />
        </div>
        <div className="card compact">
          <Stat
            label="Lifetime"
            value={lifetimeMi.toLocaleString()}
            unit="mi"
          />
        </div>
      </div>
      ...
      >
        ...
        {active.map((s) => {
          ...
          const avgSpeed =
            stats.timeSum > 0 ? stats.distSum / stats.timeSum : 0;
          ...
          return (
            <div key={s.id} className="card">
              ...
              >
                ...
                <Stat
                  label="Avg pace"
                  value={avgSpeed > 0 ? speedToPacePerMile(avgSpeed) : "—"}
                  unit="/mi"
                />
                <Stat
                  label="Last run"
                  value={stats.lastRun ? stats.lastRun.slice(5) : "—"}
                />
                <Stat
                  label="Fastest"
                  value={
                    stats.fastestSpeed > 0
                      ? paceFromSpeed(stats.fastestSpeed)
                      : "—"
                  }
                  unit="/mi"
                />
              ...
              >
                <div className="row between" style={{ marginBottom: 4 }}>
                  ...
                  <span className="num muted" style={{ fontSize: 11 }}>
                    {formatMilesHalf(last12wMi)} mi
                  </span>
                </div>
                <Sparkline
                  data={stats.weekly.length ? stats.weekly : [0]}
                  height={28}
                  stroke={s.color}
                />
              </div>
            ...
            <table className="tbl">
              ...
              <tbody>
                {retired.map((s) => {
                  const stats = perShoe.get(s.id);
                  const avgSpeed =
                    stats && stats.timeSum > 0
                      ? stats.distSum / stats.timeSum
                      : 0;
                  return (
                    <tr key={s.id}>
                      <td style={{ width: 24 }}>
                      ...
                      <td className="num">
                        {avgSpeed > 0 ? speedToPacePerMile(avgSpeed) : "—"}
                      </td>
                    </tr>
                  );
                })}
