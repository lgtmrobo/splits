...
export default async function DashboardPage() {
  ...
  const currentWeek =
    weekMileage[planMeta.current_week_index] ??
    weekMileage[weekMileage.length - 1] ??
    null;
  const todayISO = todayLocalISO();
  const weekStartISO = currentWeek?.start_date ?? todayISO;
  const [weekStats, weekView] = await Promise.all([
    getWeekStats(weekStartISO),
    getWeekView(weekStartISO),
  ]);

  const thisWeekMi = metersToMiles(weekStats.distance_m);
  const thisWeekTargetMi = currentWeek
    ? metersToMiles(currentWeek.planned_m)
    : 0;
  const thisWeekProgressPct =
    thisWeekTargetMi > 0
      ? Math.round((thisWeekMi / thisWeekTargetMi) * 100)
      : 0;
  const deficitMi = Math.max(0, thisWeekTargetMi - thisWeekMi);

  const todayPlan =
    weekView.find((d) => d.date_iso === todayISO)?.planned ?? null;
  const nextUp =
    todayPlan ??
    weekView.find((d) => d.date_iso > todayISO && d.planned)?.planned ??
    null;
  ...
  const typeByDate = new Map<string, WorkoutType>(
    plannedForRecent.map((p) => [p.scheduled_date, p.workout_type]),
  );
  ...
  const monthThis = monthly[monthly.length - 1] ?? {
    label: "—",
    miles: 0,
    runs: 0,
  };

  const peakWeek = weekMileage.reduce<{ label: string; miles: number } | null>(
    (best, w) => {
  ...
  const sessAdh =
    adherence.sessions.due > 0
      ? Math.round((adherence.sessions.done / adherence.sessions.due) * 100)
      : 0;
  const volAdh =
    adherence.volume.planned_m > 0
      ? Math.round(
          (adherence.volume.actual_m / adherence.volume.planned_m) * 100,
        )
      : 0;
  const workAdh =
    adherence.workouts.due > 0
      ? Math.round((adherence.workouts.done / adherence.workouts.due) * 100)
      : 0;
  ...
  const paceDeltaSec =
    paceFirst > 0 && paceCurrent > 0
      ...
      : null;
  const fmtPaceMinDec = (v: number) => {
    const m = Math.floor(v);
    const s = Math.round((v - m) * 60);
    if (s === 60) return `${m + 1}:00`;
    return `${m}:${String(s).padStart(2, "0")}`;
  };
  const paceLabel = paceCurrent > 0 ? fmtPaceMinDec(paceCurrent) : "—";

  const paceNonZero = paceTrend.filter((p) => p > 0);
  let lastSeen = paceNonZero[0] ?? 0;
  const paceFilled = paceTrend.map((p) => {
    if (p > 0) lastSeen = p;
    return lastSeen;
  });
  const paceMin = paceNonZero.length > 0 ? Math.min(...paceNonZero) : 0;
  const paceMax = paceNonZero.length > 0 ? Math.max(...paceNonZero) : 1;
  const pacePad = Math.max((paceMax - paceMin) * 0.2, 0.25);
  const paceMinY = Math.max(0, paceMin - pacePad);
  const paceMaxY = paceMax + pacePad;
  ...
  const remainingMi = nearRetirement
    ? Math.round(
        metersToMiles(nearRetirement.cap_m - nearRetirement.distance_m),
      )
    : 0;

  const raceDays = nextRace ? daysUntil(nextRace.race_date) : null;
  const raceWeeks =
    raceDays != null ? Math.max(0, Math.round(raceDays / 7)) : null;
  ...
  return (
    <div className="content fadein">
      ...
      <div
        className="grid"
        style={{ gridTemplateColumns: "1.4fr 1fr 1fr", marginBottom: 14 }}
      >
        <div className="card">
          <div className="row between" style={{ marginBottom: 16 }}>
            <div className="col gap-4">
              <div className="stat-label" style={{ marginBottom: 0 }}>
                This Week ·{" "}
                {currentWeek ? fmtDateRange(currentWeek.start_date) : "—"}
              </div>
              <div className="row baseline gap-10">
                <span className="stat-num xl">{thisWeekMi.toFixed(1)}</span>
                <span className="stat-unit" style={{ fontSize: 13 }}>
                  mi
                </span>
                <span
                  className="muted num"
                ...
                {deficitMi > 0 && (
                  <Pill kind="warn">−{deficitMi.toFixed(1)} mi to target</Pill>
                )}
              </div>
            </div>
            <Ring
              pct={thisWeekProgressPct}
              size={96}
              stroke={6}
              label="progress"
              value={thisWeekProgressPct}
              unit="%"
            />
          </div>
          ...
          >
            <Stat
              label="Duration"
              value={formatDurationShort(weekStats.duration_s)}
            />
            <Stat
              label="Elevation"
              value={formatFeet(weekStats.elev_m)}
              unit="ft"
            />
            <Stat
              label="Avg HR"
              value={weekStats.avg_hr > 0 ? String(weekStats.avg_hr) : "—"}
              unit="bpm"
            />
        ...
        <div className="card">
          <CardHeader
            ...
            action={
              plan ? `W${planMeta.current_week_index + 1} · ${plan.name}` : "—"
            }
          />
          <div className="row gap-14" style={{ alignItems: "center" }}>
            <Ring
              pct={planMeta.adherence_pct}
              size={84}
              stroke={6}
              label="on plan"
              value={planMeta.adherence_pct}
              unit="%"
            />
            <div className="col gap-8 grow">
              {[
                {
                  label: "Sessions",
                  value: `${adherence.sessions.done}/${adherence.sessions.due}`,
                  pct: sessAdh,
                },
                {
                  label: "Volume",
                  value: `${Math.round(metersToMiles(adherence.volume.actual_m))}/${Math.round(metersToMiles(adherence.volume.planned_m))}`,
                  pct: volAdh,
                },
                {
                  label: "Workouts",
                  value: `${adherence.workouts.done}/${adherence.workouts.due}`,
                  pct: workAdh,
                },
              ].map((r) => (
        ...
        >
          <CardHeader
            title={
              nextUp ? `Next · ${fmtDate(nextUp.scheduled_date)}` : "Next · —"
            }
            accent
          />
      ...
      <div
        className="grid"
        style={{ gridTemplateColumns: "1.8fr 1fr 0.9fr", marginBottom: 14 }}
      >
        <div className="card">
          <CardHeader
            title="Daily Load · Last 28 Days"
            action={`MIN · 0   MAX · ${loadMax}`}
          />
        ...
        <div className="card">
          ...
          <Stat
            label="Distance"
            value={monthThis.miles.toFixed(1)}
            unit="mi"
            size="lg"
          />
          <div style={{ height: 14 }} />
      ...
      >
        <div className="card">
          <div className="row between" style={{ marginBottom: 12 }}>
            <div className="col gap-4">
              ...
              <div className="row gap-14" style={{ marginTop: 4 }}>
                <span className="pill-dot" style={{ color: "var(--accent)" }}>
                  Actual
                </span>
                <span className="pill-dot" style={{ color: "var(--text-3)" }}>
                  Planned
                </span>
                {peakWeek && (
                  <span className="muted num" style={{ fontSize: 11 }}>
                    · Peak: {peakWeek.label} ({peakWeek.miles.toFixed(0)}mi)
                  </span>
                )}
              </div>
            </div>
          ...
          <MileageBars
            ...
            width={820}
            height={180}
            currentIndex={planMeta.current_week_index}
          />
        </div>
        ...
        <div className="card">
          ...
          >
            <div className="card-title">Easy Pace · 12w Trend</div>
            <div
              className="col"
              style={{ alignItems: "flex-end", gap: 2, lineHeight: 1.1 }}
            >
              <div
              ...
              {paceDeltaSec != null && (
                ...
                >
                  {paceDeltaSec >= 0 ? "−" : "+"}
                  {Math.abs(paceDeltaSec)}s vs 12w
                </span>
              )}
            </div>
          </div>
            ...
            <Stat
              ...
              value={
                recovery?.resting_heart_rate != null
                  ? String(Math.round(Number(recovery.resting_heart_rate)))
                  : "—"
              }
              unit="bpm"
            />
            <Stat
              ...
              value={
                recovery?.hrv_rmssd_milli != null
                  ? String(Math.round(Number(recovery.hrv_rmssd_milli)))
                  : "—"
              }
              unit="ms"
            />
      ...
      <div className="grid" style={{ gridTemplateColumns: "1.5fr 1fr 1fr" }}>
        <div className="card" style={{ padding: 0 }}>
          <div className="row between" style={{ padding: "16px 20px 10px" }}>
            ...
            <Link href="/activities" className="card-action">
              View all →
            </Link>
          </div>
          <table className="tbl">
            ...
            <tbody>
              {recent.map((a, i) => {
                const plannedType = typeByDate.get(
                ...
                return (
                  <tr key={a.id} className="clickable">
                    ...
                    <td className="num">
                      {speedToPacePerMile(a.average_speed_ms)}
                    </td>
                    <td className="num">{a.average_heartrate ?? "—"}</td>
                    <td className="num">
                      {formatFeet(a.total_elevation_gain_m)}
                    </td>
                  </tr>
                );
              })}
          ...
          <div className="col gap-12">
            {sortedGear.slice(0, 3).map((s) => (
              <div key={s.id} className="col gap-6">
                ...
                <MilesBar
                  miles={metersToMiles(s.distance_m)}
                  cap={metersToMiles(s.cap_m)}
                  color={s.color}
                />
              </div>
            ))}
            ...
            >
              <div className="row gap-8 baseline">
                ...
                <span className="pill warn">
                  {nearRetirement.name} approaching retirement — {remainingMi}mi
                  left
                </span>
              </div>
            </div>
        ...
        {nextRace && (
          <div className="card">
            <CardHeader
              title="Next Race"
              action={raceWeeks != null ? `${raceWeeks} weeks` : ""}
            />
            <div className="col gap-6">
              ...
              <div className="row gap-8 baseline">
                <span className="muted num" style={{ fontSize: 11 }}>
                  {fmtDate(nextRace.race_date)}
                  {nextRace.location ? ` · ${nextRace.location}` : ""}
                </span>
                {nextRace.priority && (
                  <Pill kind="accent">{nextRace.priority}</Pill>
                )}
              </div>
            </div>
            <div
            ...
            >
              <Stat
                label="Distance"
                value={formatMiles(nextRace.distance_m, 1)}
                unit="mi"
              />
              <Stat label="Goal" value={fmtGoalTime(nextRace.goal_time_s)} />
              <Stat
                label="Pace"
                value={paceFromGoal(nextRace.distance_m, nextRace.goal_time_s)}
                unit="/mi"
              />
            </div>
          </div>
